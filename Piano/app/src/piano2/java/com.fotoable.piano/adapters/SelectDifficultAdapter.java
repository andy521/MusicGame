...
public class SelectDifficultAdapter extends ZoomHoverAdapter<Map<String, Object>> {
    ...
    public View getView(ViewGroup parent, int position, Map<String, Object> s) {
        ...
        if (position == 0){
            ...
            star.setStarResource(R.drawable.tag_star0, R.drawable.tag_star1);
        }else if (position == 1){
            img_unlock.setVisibility(View.GONE);
            star.setStarResource(R.drawable.silver_star_fg, R.drawable.silver_star_bg);

        } else if (position == 2){
            star.setStarResource(R.drawable.gold_star_fg, R.drawable.gold_star_bg);

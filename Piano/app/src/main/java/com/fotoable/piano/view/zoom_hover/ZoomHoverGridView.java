...
import com.fotoable.piano.utils.DensityUtils;
...
public class ZoomHoverGridView extends GridLayout implements View.OnClickListener, ZoomHoverAdapter.OnDataChangedListener {
    ...
    public ZoomHoverGridView(Context context, AttributeSet attrs) {
        ...
        mAnimZoomTo = typedArray.getFloat(R.styleable.ZoomHoverView_zhv_zoom_to, 1.1f);
    ...
    public void onClick(View view) {
        if (mCurrentView == null) {
            ...
            if (mOnItemSelectedListener != null) {
                mOnItemSelectedListener.onItemSelected(mCurrentView, mCurrentView.getId() - 1, true);
            }
        } else {
    ...
    public void setSelectedItem(int position) {
        ...
        if (mCurrentView != null) {
            if (mCurrentView.getId() - 1 != position) {
                ...
                zoomInAnim(mCurrentView);
                if (mOnItemSelectedListener != null) {
                    mOnItemSelectedListener.onItemSelected(mCurrentView, mCurrentView.getId() - 1, false);
                }
            } else { //后期修改
                mCurrentView = null;
    ...
    public void setClickable(boolean clickable) {
        ...
        if (getChildCount() > 0) {
            for (int i = 0; i < getChildCount(); i++) {
                Log.d(TAG, "child clickable-->>" + clickable);

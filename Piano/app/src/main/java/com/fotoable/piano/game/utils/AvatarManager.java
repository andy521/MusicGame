...
public class AvatarManager {
    ...
    public float scaleX=0.04f;
    public final float scaleY=0.04f;

    public RectF  pauseRectF;
    public AvatarManager(GameRender gameRender) {
    ...
    public void init() {
        ...
        float marginTop = 0.06f+ProgressBarManager.scaleY+scaleY;
        float marginLeft = 0.06f+scaleY;
        translateY = (GLConstants.getScreenHeight()/2-marginTop*GLConstants.getScreenHeight())/(GLConstants.getScreenHeight()/2);
        translateX = -(GLConstants.getScreenRatioWh()-marginLeft);
        pauseRectF = new RectF();
        pauseRectF.top=0;
        pauseRectF.left=0;
        pauseRectF.right=160;
        pauseRectF.bottom=160;
    }

    private void initTexture() {
        texture = new TextureMode();
        texture.pictureName = "avatar_girl.jpg";
        GLUtils.loadTexture(texture, true,true);
    ...
    protected void initBuffer() {
        verBuffer = GLUtils.CreateVertexArray(GLUtils.pos);//position Buffer
        texBuffer = GLUtils.CreateVertexArray(GLUtils.coord);// 纹理buffer
    }

    public void drawAvatar() {

//        Log.d(TAG, "drawPauseBtn");
...
//        Log.e(TAG,"pointerIndex-->>"+pointerIndex+", coor[0]-->>"+coor[0]+", -coor[1]-->>"+(-coor[1]));
        ...
        //准备三角形的坐标数据
        GLES20.glVertexAttribPointer(positionNode, 2, GLES20.GL_FLOAT, false, 0, verBuffer);
     ...
     * @param touchY y
     */
    public boolean  checkInPauseBtnArea(float touchX,float touchY){

        boolean result =touchY>=pauseRectF.top&&touchY<=pauseRectF.bottom&&touchX>=pauseRectF.left&&touchX<=pauseRectF.right;
